...
class ScenarioManager:
    ...
    def __init__(self, time_horizon_days: float, dt_days: float = 0.1):
        ...
        # Using MLX for time array
        self.t_array = mx.linspace(0, time_horizon_days, self.time_steps)
    ...
    def generate_scenarios(self, base_temp_k: float, temp_var: float, 
        ...
        """
        # MLX accelerated random normal generation
        # Temp shape: (num_scenarios, time_steps)
        self.temps = base_temp_k + temp_var * mx.random.normal((num_scenarios, self.time_steps))
        
        # RH shape: (num_scenarios, time_steps), clamped between 0 and 1
        rh_raw = base_rh + rh_var * mx.random.normal((num_scenarios, self.time_steps))
        self.rhs = mx.clip(rh_raw, 0.0, 1.0)
